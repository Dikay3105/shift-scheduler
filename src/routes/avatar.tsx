...
function AvatarGeneratorPage() {
    ...
    function flwr(c: CanvasRenderingContext2D, cx: number, cy: number, r = 16) {
        c.save();
    ...
    function brry(c: CanvasRenderingContext2D, cx: number, cy: number, r = 6) {
        c.save();
    ...
    function shieldPath(
        ...
        c.quadraticCurveTo(bx + sw * 0.38, by + sh * 0.52, bx, by + sh * 0.58);
        c.quadraticCurveTo(bx - sw * 0.38, by + sh * 0.52, bx - sw * 0.47, by + sh * 0.09);
        c.closePath();
    }
    ...
    function drawPhoto(c: CanvasRenderingContext2D) {
        ...
        if (img) {
            ...
            c.drawImage(img, sx, sy, sw, sh, CX - PR, CY - PR, PR * 2, PR * 2);
        } else {
            c.fillStyle = "#DDD9D0";
            c.fill();
    ...
    function drawWreath(c: CanvasRenderingContext2D) {
        ...
        c.beginPath();
        c.moveTo(LE.x, LE.y);
        c.bezierCurveTo(LE.x - 2, LE.y + 22, CX - 40, BY - 12, CX - 48, BY - 2);
        c.stroke();

        c.beginPath();
        c.moveTo(RE.x, RE.y);
        c.bezierCurveTo(RE.x + 2, RE.y + 22, CX + 40, BY - 12, CX + 48, BY - 2);
        c.stroke();
        c.restore();

        leaf(c, CX - 44, BY - 22, 2.7, 17);
        leaf(c, CX - 34, BY - 8, 2.4, 14);
        leaf(c, CX + 44, BY - 22, 0.42, 17);
        leaf(c, CX + 34, BY - 8, 0.72, 14);
        ...
        for (let ad = 110; ad <= 428; ad += 12) {
            ...
            leaf(c, x, y, rad + (Math.floor(ad / 12) % 2 === 0 ? 0.4 : -0.4), 17);
        }
    ...
    function drawText(c: CanvasRenderingContext2D) {
        ...
        const by = CY + 320;
        const sh = 70;
        const badgeBottom = by + sh * 0.58; // đáy badge
        const nameY = badgeBottom + 40;
        const roleY = nameY + 50;

        if (name) {
            c.save();
            c.font = "italic 52px Georgia";
            c.textAlign = "center";
            c.textBaseline = "middle";
            c.fillStyle = "#2C2C2C";
            c.fillText(name, CX, nameY);
            c.restore();
        }
                    ...
                    <div className="grid items-stretch gap-6 lg:grid-cols-[380px_1fr]">
                        ...
                        <div className="flex flex-col rounded-3xl bg-white p-5 shadow-2xl">
                            ...
                            <div className="mb-4">
                                ...
                                <label className="flex cursor-pointer items-center gap-3 rounded-2xl border-2 border-dashed border-[#c5b8a5] p-3 text-left transition-all duration-300 hover:bg-[#f8f1e9]">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <p className="truncate text-sm font-semibold">Kéo thả ảnh vào đây</p>
                                        <p className="truncate text-xs text-muted-foreground">hoặc nhấn để chọn ảnh</p>
                                    </div>
                                    <input
                                        type="file"
                                        accept="image/*"
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="mb-2 block text-sm font-medium">Tên nhân viên</label>
                                    <input
                                        value={name}
                                ...
                                <div>
                                    <label className="mb-2 block text-sm font-medium">Chức vụ</label>
                                    <input
                                        value={role}
